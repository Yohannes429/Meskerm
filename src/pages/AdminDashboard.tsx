...
import { Shield, Plus, Edit, Trash2, Newspaper, Eye, EyeOff, Loader2, Users, CalendarDays, FileText, Megaphone } from "lucide-react";
...
const AdminDashboard = () => {
  ...
  const checkAuth = async () => {
    ...
    if (role !== "admin") { toast.error("Unauthorized"); navigate("/"); return; }
  ...
  const openCreateDialog = () => {
    setEditingPost(null); setTitle(""); setContent(""); setExcerpt(""); setCategory("General"); setImageFile(null);
    setDialogOpen(true);
  };

  const openEditDialog = (post: NewsPost) => {
    setEditingPost(post); setTitle(post.title); setContent(post.content);
    setExcerpt(post.excerpt || ""); setCategory(post.category); setImageFile(null);
    setDialogOpen(true);
  };
  ...
  const handleSave = async () => {
    ...
    setDialogOpen(false); setSaving(false); await fetchPosts();
  };

  const togglePostStatus = async (post: NewsPost) => {
    const newStatus = post.status === "published" ? "draft" : "published";
    const { error } = await supabase.from("news_posts" as any).update({ status: newStatus } as any).eq("id", post.id);
    if (error) toast.error("Failed to update status");
    else { toast.success(`Post ${newStatus}`); await fetchPosts(); }
  };

  const deletePost = async (id: string) => {
    const { error } = await supabase.from("news_posts" as any).delete().eq("id", id);
    if (error) toast.error("Failed to delete post"); else { toast.success("Post deleted"); await fetchPosts(); }
  };
      ...
      <main className="container mx-auto px-4 py-4 pb-12">
        <Tabs defaultValue="posts" className="space-y-4">
          <TabsList className="flex-wrap h-auto gap-1">
            <TabsTrigger value="posts" className="gap-1"><Newspaper className="h-3 w-3" />Posts</TabsTrigger>
            <TabsTrigger value="employees" className="gap-1"><Users className="h-3 w-3" />Employees</TabsTrigger>
          ...
          <TabsContent value="posts" className="space-y-4">
            <div className="flex justify-between items-center">
              <div className="grid gap-4 grid-cols-3 flex-1 mr-4">
                <Card><CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2"><CardTitle className="text-sm font-medium">Total</CardTitle><Newspaper className="h-4 w-4 text-primary" /></CardHeader><CardContent><div className="text-2xl font-bold">{posts.length}</div></CardContent></Card>
                <Card><CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2"><CardTitle className="text-sm font-medium">Published</CardTitle><Eye className="h-4 w-4 text-secondary" /></CardHeader><CardContent><div className="text-2xl font-bold">{publishedPosts.length}</div></CardContent></Card>
                <Card><CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2"><CardTitle className="text-sm font-medium">Drafts</CardTitle><EyeOff className="h-4 w-4 text-muted-foreground" /></CardHeader><CardContent><div className="text-2xl font-bold">{draftPosts.length}</div></CardContent></Card>
              </div>
              <Button onClick={openCreateDialog} className="shrink-0"><Plus className="h-4 w-4 mr-2" />New Post</Button>
            </div>
                ...
                <Card key={post.id}>
                  <CardHeader>
                    <div className="flex items-start justify-between gap-4">
                      ...
                      {post.image_url && <img src={post.image_url} alt="" className="h-20 w-20 rounded-lg object-cover shrink-0" />}
                    </div>
                  </CardHeader>
                  <CardContent>
                    <div className="flex gap-2 flex-wrap">
                      <Button size="sm" variant="outline" onClick={() => openEditDialog(post)}><Edit className="h-4 w-4 mr-1" />Edit</Button>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          ...
          <div className="space-y-4">
            <div className="space-y-2"><Label htmlFor="post-title">Title *</Label><Input id="post-title" value={title} onChange={e => setTitle(e.target.value)} placeholder="Post title" /></div>
            <div className="space-y-2"><Label htmlFor="post-excerpt">Excerpt</Label><Input id="post-excerpt" value={excerpt} onChange={e => setExcerpt(e.target.value)} placeholder="Short summary" /></div>
            ...
            <div className="space-y-2"><Label htmlFor="post-content">Content *</Label><Textarea id="post-content" value={content} onChange={e => setContent(e.target.value)} placeholder="Write your post content..." rows={8} /></div>
            <div className="space-y-2">
              <Label htmlFor="post-image">Image (optional)</Label>
              <Input id="post-image" type="file" accept="image/*" onChange={e => setImageFile(e.target.files?.[0] || null)} />
              {editingPost?.image_url && !imageFile && <img src={editingPost.image_url} alt="" className="h-24 rounded-lg object-cover mt-2" />}
            </div>
          </div>
          <DialogFooter>
